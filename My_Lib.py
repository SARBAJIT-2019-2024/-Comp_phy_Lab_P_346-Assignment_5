...
def crout_method(a):
    ...
    for i in range(n):
        for j in range(i,n):
            sum_lower = 0
            for k in range(i):
                sum_lower += (a[i][k]*a[k][j]) #conditions for lower triangular sum
        ...
        for j in range(i,n):
            if (i == j):
                ...
                sum_upper = 0
                for k in range(i):
                    sum_upper += (a[j][k]*a[k][i]) #condition for upper triangular sum
...
def forward_backward_sub_do_little(Upper_tri, Lower_tri, b):
    ...
    for i in range(n):
        total = 0
        for j in range(i):
            total += Lower_tri[i][j] * y[j]#summation part
        y[i] = b[i] - total
    ...
    for i in reversed(range(n)):
        total = 0
        for j in range(n):
            total += Upper_tri[i][j] * x[j]#summation part for continuous iterations
...
def forward_backward_sub_crout(Upper_tri, Lower_tri, b):
    n=len(Upper_tri)
    partial_pivoting(Upper_tri,Lower_tri,n)
    D= combiner(Upper_tri,Lower_tri) #combiner of two matrices
    ...
    for i in range(n):
        total = 0
        for j in range(i):
            total += D[i][j] * y[j] #summation part
        y[i] = b[i] - total
    ...
    for i in reversed(range(n)):
        total = 0
        for j in range(n):
            total += D[i][j] * x[j] #summation part for continuous iterations
...
def linear_solver_crout(list_C):
    ...
    for i in range(len(A)):
        if A[i][i]==0:
            print("No solutions exist for this system !") #Checking for diagonal entries
            return None
...
def Do_little(a):
    ...
    for j in range(n):
        for i in range(n):
            total = 0
            for k in range(i):
                total +=Lower_tri[i][k] * Upper_tri[k][j]  #summation part

            if i == j:
                Upper_tri[i][j] = a[i][j] - total

            elif i > j:
                Lower_tri[i][j] = (a[i][j] - total)/Upper_tri[j][j] #Summation part
...
def linear_solver_do_little(list_C):
    ...
    for i in range(len(A)):
        if A[i][i]==0:
            print("No solutions exist!") #conditioning for diagonal entries 
            return None
    Upper_tri, Lower_tri = Do_little(A) #making two parts
...
def LU_inverse(a):
    ...
    if len(a) != len(a[1]):         
        print("No solutions exist")#checking for entry of Square matrix.
        return None
    I=identity_mat(4)
    ...
    x_1=Nullmatrix(4,4)
    x_2=Nullmatrix(4,4)  #Null matrices for storing


    for i in range(4):
	    x_1[i]=forward_backward_sub_crout(Upper_tri,Lower_tri,I[i])# storing matrix
...
def forward_backward_sub_cholesky(Lower_tri, Lower_tri_Transpose, b):
    ...
    y = [0 for i in range(n)] #storing matrix
    ...
    for i in range(n):
        ...
        y[i] = (b[i] - total)/Lower_tri[i][i]     #dividing for L

    x = [0 for i in range(n)]

    for i in reversed(range(n)): #reversed range iteration
        total = 0
        for j in range(n):
            total += Lower_tri_Transpose[i][j] * x[j]
        
        x[i] = (y[i] - total)/Lower_tri_Transpose[i][i] #Dividing for L^T
    ...
    for i in range(n):
        for j in range(i + 1):      #Fixing range for the iterations
            ...
            if j == i:                           #confition for diagonal entries
                ...
                Lower_tri[j][j] =(a[j][j] - total)**(0.5)   #taking the sqaure root
        ...
        if prod < 0:
            ...
            while prod > 0:
                if abs(f(a_i)) < abs(f(b_i)):
                    a_i = a_i - beta*(b_i-a_i)
                    custom_iteration += 1
                    prod = f(a_i) * f(b_i)

                elif abs(f(a_i)) > abs(f(b_i)):
                    b_i = b_i + beta*(b_i-a_i)
                    custom_iteration += 1
                    prod = f(a_i) * f(b_i)

            if custom_iteration > 10:
                print("Try another bracket range")
                exit()

            return a_i, b_i
# Bisection method of finding roots of a given equation
def bisection(f, a_i, b_i, precision):
    a_i, b_i = bracketing(f, a_i, b_i)
    No_of_iterations = []
    root_val_i = []
    f_x_i = []
    ...
    while abs(a_i-b_i)>precision:
        ...
        root_val_i.append(c_i)
        f_x_error = abs(root_val_i[i] )
        f_x_i.append(f_x_error)
        i+=1
    else:
        return c_i, f_x_i, No_of_iterations
