...
def bisection_method(f, a_i, b_i, precision):
    ...
    while abs(a_i-b_i)>precision:
        ...
        f_x_i.append(f(c_i))
        x_error = abs(root_val_i[i]-root_val_i[i-1])
...
#########################################################################################################
#########################################################################################################
def regula_falsi_method(f, a_i, b_i, precision):
    a_i, b_i = bracketing(f, a_i, b_i)
    No_of_iterations = []
    root_val_i = []
    x_i = []
    ...
    while abs(a_i-b_i) >= precision:
        ...
        c_i = b_i - ((b_i-a_i)*f(b_i))/(f(b_i) - f(a_i))
        if f(c_i)==0:
            return c_i, x_i,f_x_i, No_of_iterations
        if f(a_i) * f(c_i) < 0:
            b_i = c_i
...
def differentiation(f,x_0):
    ...
    h=10**(-10)
    f1_x_0 = (f(x_0+h) - f(x_0-h))/(2*h)
    return f1_x_0



def Newton_Raphson_Method(f, x_0, precision):
    No_of_iterations = []
    f_x_i=[]
    i=0
    
    while abs(f(x_0)/differentiation(f, x_0))>precision:
        
        x_0 = x_0 - (f(x_0)/differentiation(f, x_0))
        No_of_iterations.append(i)
        f_x_i.append(f(x_0))
        i+=1
...
def double_differentiation(f, x_0, h):
    h=10**(-10)
    f2_x_0 = ((differentiation(f,x_0)+h)-(differentiation(f,x_0)-h))/(2*h)
